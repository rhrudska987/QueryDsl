    ...
    public void search(){
        Member findMember = queryFactory.selectFrom(QMember.member)
                .where(QMember.member.username.eq("member1")
                        .and(QMember.member.age.eq(10))).fetchOne();
...
//        List<Member> content = results.getResults();
    ...
    public void aggregation(){
        List<Tuple> result = queryFactory
                .select(member.count(), member.age.sum(), member.age.avg(), member.age.max(), member.age.min()).from(member)
                .fetch();
    ...
    public void theta_join(){
        ...
        List<Member> result = queryFactory
                .select(member).from(member, team)
    ...
    public void join_on_filtering(){
        List<Tuple> result = queryFactory
                ...
                .on(team.name.eq("teamA")) //내부조인의 경우 where(team.name.eq("teamA"))로 하고, 외부조인의 경우 on을 사용
                .fetch();
    ...
    public void concat(){
        //{username}_{age}
        List<String> result = queryFactory
                .select(member.username.concat("_").concat(member.age.stringValue())) //stringvalue는 enum을 처리할 때, 자주 사용
    ...
    @Test
    public void findDtoBySetter(){ //MemberDto에 @Data가 있거나 @Setter가 있어야 함
    ...
    @Test
    public void findDtoByField() { //Getter, Setter 둘 다 필요없음
        List<MemberDto> result = queryFactory
    ...
    @Test
    public void findDtoByConstructor() { //생성자 필요, //단점은 생성자에 값 추가시 컴파일 오류를 못잡고 런타임 에러가 발생함
        List<MemberDto> result = queryFactory
    ...
    public void bulkUpdate(){
        ...
        long count = queryFactory
                ...
                .execute();
        //영속성 컨텍스트에는 밑에 결과대로 남음..
        //member1 = 10 -> DB member1
        //member2 = 20 -> DB member2
        //member3 = 30 -> DB member3
        //member4 = 40 -> DB member4
        //따라서, flush, clear을 해야함
        em.flush();
        em.clear();
    }
